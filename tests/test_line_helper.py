...
class TestLineHelper:
    ...
    async def test_get_by_mode(self, mock_session, mock_lines_data):
        ...
        result = await helper.get_by_mode(TransportMode.METRO)
        assert len(result) == 2
        assert all(ln.transport_mode == TransportMode.METRO for ln in result)

        result = await helper.get_by_mode(TransportMode.BUS)
        assert len(result) == 2
        assert all(ln.transport_mode == TransportMode.BUS for ln in result)
    ...
    async def test_get_by_designation(self, mock_session, mock_lines_data):
        ...
        assert result is not None
        assert result.transport_mode == TransportMode.BUS
    ...
    async def test_sorting(self, mock_session, mock_lines_data):
        ...
        bus_lines = [ln for ln in result if ln.transport_mode == TransportMode.BUS]
...
class TestLineHelperIntegration:
    ...
    async def test_get_metro_lines(self, session):
        ...
        helper = LineHelper(session)
        metro = await helper.get_by_mode(TransportMode.METRO)

        assert len(metro) > 0
        assert all(ln.transport_mode == TransportMode.METRO for ln in metro)
